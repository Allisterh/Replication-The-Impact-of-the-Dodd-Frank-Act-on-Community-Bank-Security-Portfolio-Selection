######### Code for Dodd-Frank Act Project ######################
################################################################
########## Alexander Wirth ####################################
...
#Setting Working Directory
    setwd("C:/Users/amwirth/Google Drive/Research/939A Dodd Frank/Call_Report_Data")
...
#Fucntion Creating Quarter End Dates for Reading in Time Series of Data
  ...
  for (i in 1:length(dates_vector)){
    ...
    path=paste(getwd(),"/",dates_vector[i],"/",
  ...
  for (i in 1:length(dates_vector)) {
    ...
    if (dates_vector[i] < "06302009") 
      {
      path1=paste(getwd(),"/",dates_vector[i],"/",
    ...
    RCB1[[i]] <- read.table(path1, header = FALSE, fill = TRUE, sep = "\t", skip = 2)
    names(RCB1[[i]]) <- Headers_RCB1_Names[[i]]
    path2=paste(getwd(),"/",dates_vector[i],"/",
...
### Making Data Tidy for GGplot
tidy_data <- gather(By_Quarter_Percentage_Securities_NonGov, key = "Variable", value = "Percentage", -Quarter)




ggplot(tidy_data %>% filter(Variable != "MunisPercentage"), aes(x = Quarter, y = Percentage, color = Variable, 
                                                                group = Variable)) + 
  geom_line() + 
...
####### Ad-Hoc Way Rest of the way not in tidy data format, I like the look a little better
...
#########################################################################
#########################################################################
### Moedling Detaisl to Tobit Censored Models Estimated with Maximium Liklihood Estimation 
